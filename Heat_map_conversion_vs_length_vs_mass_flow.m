...
L_range = 1:1:15;
m_flowrange = 0.0005:0.001:0.005;

HCHO_flowrate = zeros(length(m_flowrange), length(L_range));
...
for i = 1:length(m_flowrange)
    ...
    for j = 1:length(L_range)
        L = L_range(j); 
        [z_points, F_CH3OH, F_O2, F_HCHO, F_H2O, F_CO, F_N2, extent1, extent2, P, T]=solve_reactor(m_flow,L, P0, T0, option);
        F_out = F_HCHO(end);
        HCHO_flowrate(i,j) = F_out;
    end
end

figure
imagesc(L_range, m_flowrange, HCHO_flowrate)
set(gca,'YDir','normal')
xlim
xlabel('Reactor Length (m)')
ylabel('Mass Flow per Tube (kg/s)')
title('HCHO Outlet Flowrate Heat Map')
