...
% ===== after you compute r1, r2 =====

% --- logging (stores values across solver calls) ---
persistent r1_log r2_log T_log P_log
if isempty(r1_log)
    r1_log = [];
    r2_log = [];
    T_log  = [];
...
% your independent variable (you named it t, but it's z)
r1_log(end+1,1) = r1;
r2_log(end+1,1) = r2;
T_log(end+1,1)  = T;
P_log(end+1,1)  = P;

% push to base workspace so you can plot after ode15s finishes
assignin('base','r1_log',r1_log);
assignin('base','r2_log',r2_log);
...
% legend('T')
% 
% %conversion of zpoints to wpoints
% rho_cat = 1500; %kg m^-3
% bed_voidage = 0.4;
% tube_diameter = 20 * 10^-3;
% A = pi*(tube_diameter/2)^2;
% 
% w_points = (rho_cat*(1-bed_voidage)*A).*z_points;
% 
% figure("Name", "Catalyst weight vs Extent of reaction")
% plot(w_points, extent1, "LineWidth", 2); hold on;
% plot(w_points, extent2, "LineWidth", 2);
% title("Extent vs. Catalyst Weight")
% xlabel("Catalyst Weight (kg)")
% ylabel("Extent")
% 
% [T_s, idx] = sort(T_log);
% [T_u, ia] = unique(T_s, 'stable');
% 
% r1_s = r1_log(idx);
% r1_u = r1_s(ia);
% 
...
figure("Name", "Rate 1 vs Temperature")
plot(T_u, r1_u, "LineWidth", 2);
title("Rate vs Temperature")
xlabel("Temperature")
ylabel("Rate of reaction 1")


figure("Name", "Rate 2 vs Temperature")
plot(T_u, r2_u, "LineWidth", 2);
title("Rate vs Temperature")
xlabel("Temperature")
ylabel("Rate of reaction 2")
