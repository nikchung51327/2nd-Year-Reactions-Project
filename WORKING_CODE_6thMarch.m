...
function [value, isterminal, direction] = T_max_event(z, F, T_max)
    value      = F(10) - T_max;  % triggers when T = T_max
    isterminal = 1;               % stop integration
    direction  = 1;               % only when T is increasing
end
...
function dT_dz = energy_bal(r1, r2, y, n_T, T)
    ...
    dT_dz = num / den;
% Diagnostic Lines
% fprintf('r1 = %.4e mol/kg/s\n', r1);
% fprintf('r2 = %.4e mol/kg/s\n', r2);
% fprintf('n_T = %.4e mol/s\n', n_T);
% fprintf('sum_yCp = %.4e J/mol/K\n', sum_y_Cp);
% fprintf('num = %.4e W/m\n', -(r1*Hr1 + r2*Hr2));
% fprintf('den = %.4e W/K\n', n_T*sum_y_Cp);
% fprintf('dT_dz = %.4e K/m\n', dT_dz);
end
...
function [z_points, F_CH3OH, F_O2, F_HCHO, F_H2O, F_CO, F_N2, extent1, extent2, P, T] = solve_reactor(m_flowrate,reactor_length,P0,T0,T_max,T_max_active,options)
    ...
    %toggle - if there is a maximum temperature bound 
   if T_max_active == "Yes"
    opts = odeset('RelTol', 1e-6, 'AbsTol', 1e-9, ...
                  'Events', @(z,F) T_max_event(z, F, T_max));
    else
    opts = odeset('RelTol', 1e-6, 'AbsTol', 1e-9);
    end
    ...
    % --- ADD THIS BLOCK ---
    if ~isempty(z_stop) && T_max_active == "Yes"
...
function plot_reactor_results(z_points, F_CH3OH, F_O2, F_HCHO, F_H2O, F_CO, F_N2, P, T, extent1, extent2)
    % --- Plots ---
...
r2_profile = zeros(length(z_points), 1);
    ...
    % Mol fractions
    y_CH3OH_i = F_CH3OH(i) / n_T_i;
    y_O2_i    = F_O2(i)    / n_T_i;
    y_H2O_i   = F_H2O(i)  / n_T_i;
    y_HCHO_i  = F_HCHO(i)  / n_T_i;
    
    % Partial pressures [atm]
    P_CH3OH_i = y_CH3OH_i * P(i);
    P_O2_i    = y_O2_i    * P(i);
    P_H2O_i   = y_H2O_i   * P(i);
    P_HCHO_i  = y_HCHO_i  * P(i);
    
    % Compute rates using LHHW (change to rates_powerlaw if needed)
    alpha = 0.44;
    [r1_profile(i), r2_profile(i)] = rates_LHHW(T(i), alpha, P_CH3OH_i, P_O2_i, P_H2O_i, P_HCHO_i);
    end
    ...
    xlabel('Reactor Length (m)');
    ylabel('Rate (mol/kg_{cat}/s)');
    legend('r_1 (CH_3OH \rightarrow HCHO)', 'r_2 (HCHO \rightarrow CO)');
    xlim([0 z_points(end)])
    title('Reaction Rates Across the Reactor');
    grid on;
    
    % --- Conversion Plot ---
    F_CH3OH_0 = F_CH3OH(1);  % inlet molar flowrate
    X_CH3OH = (F_CH3OH_0 - F_CH3OH) / F_CH3OH_0;
    
    figure("Name", "Conversion of CH3OH Across Reactor")
    plot(z_points, X_CH3OH, 'b-', 'LineWidth', 2)
    xlabel('Reactor Length (m)')
    ylabel('Conversion X_{CH_3OH}')
...
Pressure_drop = "Yes";
Temp_dependence = "Yes";
T_max = 800; %maximum temperature in the reactor
T_max_active = "Yes";
%----------------------------------------------------------
%----------------------------------------------------------
%----------------------------------------------------------
...
[z_points, F_CH3OH, F_O2, F_HCHO, F_H2O, F_CO, F_N2, extent1, extent2, P, T]= ...
    solve_reactor(m_flowrate,reactor_length,P0,T0,T_max,T_max_active,option);

plot_reactor_results(z_points, F_CH3OH, F_O2, F_HCHO, F_H2O, F_CO, F_N2, P, T, extent1, extent2)